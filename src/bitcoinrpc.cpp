...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "so payments received with the address will be Creditsed to [account].");
...
int64 GetAccountBalance(CWalletDB& walletdb, const string& strAccount, int nMinDepth)
{
    ...
    nBalance += walletdb.GetAccountCreditsDebit(strAccount);
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    // Credits
    CAccountingEntry Credits;
    Credits.strAccount = strTo;
    Credits.nCreditsDebit = nAmount;
    Credits.nTime = nNow;
    Credits.strOtherAccount = strFrom;
    Credits.strComment = strComment;
    walletdb.WriteAccountingEntry(Credits);
...
void AcentryToJSON(const CAccountingEntry& acentry, const string& strAccount, Array& ret)
{
    ...
    if (fAllAccounts || acentry.strAccount == strAccount)
    {
        ...
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nCreditsDebit)));
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    walletdb.ListAccountCreditsDebit(strAccount, acentries);
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    list<CAccountingEntry> acentries;
    CWalletDB(pwalletMain->strWalletFile).ListAccountCreditsDebit("*", acentries);
    BOOST_FOREACH(const CAccountingEntry& entry, acentries)
        mapAccountBalances[entry.strAccount] += entry.nCreditsDebit;
...
Value gettransaction(const Array& params, bool fHelp)
{
    ...
    int64 nCredits = wtx.GetCredits();
    int64 nDebit = wtx.GetDebit();
    int64 nNet = nCredits - nDebit;
